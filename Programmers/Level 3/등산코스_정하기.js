...
const makeCombination = (gates, summits) => {
    const combination = [];
    for (let i = 0; i < gates.length; i++) {
        for (let j = 0; j < summits.length; j++) {
            combination.push([gates[i], summits[j]]);
        }
    }
...
function solution(n, paths, gates, summits) {
    ...
    for (const [gate, summit] of makeCombination(gates, summits)) {
        const result = [];
        ...
        while (queue.length > 0) {
            ...
            if (point === summit) {
                result.push(Math.max(...pathWeights));
                continue;
            }
            ...
            for(const {next: connectedPoint, weight} of graph.get(point)) {
                ...
                if(!visited.has(connectedPoint)) {
                    queue.push([connectedPoint, [...pathWeights, weight], new Set([...visited.values(), connectedPoint])]);
                }
            }
        }

        answer.push([summit, Math.min(...result)]);
    }

    return answer.sort(([summit1, intensity1], [summit2, intensity2]) => 
...
console.log(solution(7, 	[[1, 2, 5], [1, 4, 1], [2, 3, 1], [2, 6, 7], [4, 5, 1], [5, 6, 1], [6, 7, 1]], [3, 7], [1, 5])); //[5,1]
